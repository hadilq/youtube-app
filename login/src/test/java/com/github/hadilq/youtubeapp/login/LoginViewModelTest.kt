...
import com.github.hadilq.youtubeapp.login.di.FakeLoginModule
import com.github.hadilq.youtubeapp.login.util.test
...
internal class LoginViewModelTest {
  ...
  fun `loginPlease would chooseAccount when ConnectionResult is SUCCESS`() = runBlockingTest {
    val anyConnectionResult: ConnectionResult = com.google.android.gms.common.ConnectionResult.SUCCESS
    with(FakeLoginModule(this)) {
      with(LoginViewModel()) {
        onBorn()
  ...
  fun `loginPlease would show just loading when PlayService is not available`() = runBlockingTest {
    val anyConnectionResult: ConnectionResult = com.google.android.gms.common.ConnectionResult.CANCELED
    with(FakeLoginModule(this)) {
      with(LoginViewModel()) {
        onBorn()
        val loadingTest = loading.test(this@runBlockingTest)
  ...
  @Test
  fun `loginPlease would show error dialog when PlayService is not available and user has error`() = runBlockingTest {
    val anyConnectionResult: ConnectionResult = com.google.android.gms.common.ConnectionResult.CANCELED
    with(FakeLoginModule(this)) {
      with(LoginViewModel()) {
        onBorn()
        val loadingTest = loading.test(this@runBlockingTest)
        val showErrorDialogTest = showGooglePlayServicesAvailabilityErrorDialog.test(this@runBlockingTest)
        with(isGooglePlayServicesAvailable) { coEvery { execute() } returns anyConnectionResult }
        ...
        with(isGooglePlayServicesAvailable) { coVerify { execute() } }
        with(isGoogleUserResolvableError) { coVerify { execute(anyConnectionResult) } }
        showErrorDialogTest.assertValues(anyConnectionResult)
        loadingTest.assertValues(true, false)
        onDie()
        loadingTest.finish()
  ...
  @Test
  fun `loginPlease would launch intent when UserRecoverableAuthIOError happened`() = runBlockingTest {
    val anyConnectionResult: ConnectionResult = com.google.android.gms.common.ConnectionResult.SUCCESS
    val anyAccountName = "AnyAccountName"
    val intent = Intent()
    with(FakeLoginModule(this)) {
      with(LoginViewModel()) {
        ...
        with(loadChannels) {
          coEvery { execute() } returns
            flowOf(Right<List<Channel>, Error>(UserRecoverableAuthIOError(RuntimeException(), intent)))
        }
  ...
  @Test
  fun `loginPlease would launch intent when GooglePlayServicesAvailabilityError happened`() = runBlockingTest {
    val anyConnectionResult: ConnectionResult = com.google.android.gms.common.ConnectionResult.SUCCESS
    val anyAccountName = "AnyAccountName"
    val intent = Intent()
    with(FakeLoginModule(this)) {
      with(LoginViewModel()) {
        ...
        with(loadChannels) {
          coEvery { execute() } returns
            flowOf(Right<List<Channel>, Error>(GooglePlayServicesAvailabilityError(RuntimeException(), intent)))
        }
  ...
  @Test
  fun `setSelectedAccountName would show just loading when PlayService is not available`() = runBlockingTest {
